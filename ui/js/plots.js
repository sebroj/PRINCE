...
function ParamPlot(event)
{
  ...
  var existingTab = chromeTabs.getTabByAlias(paramInfo["alias"]);
  if (existingTab != null)
    chromeTabs.removeTab(existingTab);

  chromeTabs.addTab({title: paramInfo["name"]}, paramInfo["alias"]);
  var $div = $("<div>", {id: TAB_PAGE_PREFIX + paramInfo["alias"],
    "class": "tab-page"});
  $("body").append($div);
  ...
  if (paramData["dim"] == 1) {
    ...
    var svg = d3.select("#" + TAB_PAGE_PREFIX + paramInfo["alias"])
      .append("svg")
  ...
  if (paramData["dim"] == 2) {
    ...
    var canvas = d3.select("#" + TAB_PAGE_PREFIX + paramInfo["alias"])
      .append("canvas")
      .attr("width", width * 10)
